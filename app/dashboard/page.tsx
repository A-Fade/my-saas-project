...
export default function Dashboard() {
  ...
  async function checkUserAndFetchData() {
    setLoading(true);
    // 1. Pehle logged-in user ki information lo
    ...
    try {
      // 2. Fetch Projects (Only for this user)
      const { data: projects } = await supabase.from("projects").select("*").eq("user_id", userId);
      const active = projects?.filter(p => p.status === "active") || [];

      // 3. Fetch Workers (Only for this user)
      const { data: workers } = await supabase.from("workers").select("*").eq("user_id", userId);

      // 4. Today's Attendance & Salary (Only for this user's projects)
      // Note: Hum attendance ko projects table se join karke filter kar rahe hain
      const { data: attToday } = await supabase.from("attendance")
        .select(`*, workers!inner(salary, user_id), projects!inner(user_id)`)
        .eq("date", todayDate)
        .eq("status", "present")
      ...
      // 5. Pending Payments (Unpaid attendance for this user)
  ...
  // Common Card Component used inside the return
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <main className="p-4 md:p-8 max-w-7xl mx-auto w-full">
        ...
        <div className="mb-8">
          <h1 className="text-3xl font-black text-slate-800 tracking-tight tracking-widest uppercase">System Dashboard</h1>
          <p className="text-slate-500 font-medium italic">Showing private data for your account</p>
        </div>
          ...
          <div className="mt-8 animate-in fade-in zoom-in duration-300">
            <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-2xl overflow-hidden">
              <div className="bg-slate-800 p-5 text-white flex justify-between items-center">
                <h3 className="font-bold uppercase tracking-widest text-sm italic">
                  {view === 'today' ? "My Present Workers Today" : view === 'pending' ? "My Pending Payments" : "My Payment History"}
                </h3>
                <button onClick={() => setView(null)} className="p-2 hover:bg-red-500 rounded-full transition"><X size={20}/></button>
              </div>
              
              <div className="p-6 max-h-[500px] overflow-y-auto">
                {/* Rendering logic remains same but now filtered by user_id */}
                {view === 'today' && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {data.todayWorkersList.length === 0 ? <p className="text-slate-400 italic">No one present today.</p> : data.todayWorkersList.map((att, i) => (
                      <div key={i} className="p-4 bg-slate-50 rounded-2xl border flex justify-between items-center">
                        <div><p className="font-bold">{att.workers?.name}</p><p className="text-[10px] text-slate-500">Project: {att.projects?.name}</p></div>
                        <span className="font-black text-blue-600">₹{att.workers?.salary}</span>
                      </div>
                    ))}
                  </div>
                )}
                {/* ... (Pending and History list maps) ... */}
                {view === 'pending' && (
